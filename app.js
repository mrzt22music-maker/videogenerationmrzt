/* ════════════════════════════════════════════════════════
   GenTube – app.js
   AI Video Generation via Cloudflare Worker Proxy → Hugging Face
   ════════════════════════════════════════════════════════ */

// ─── ⚙️  CONFIGURATION — EDIT HERE ───────────────────────
const HF_MODEL  = "camenduru/text-to-video-ms-1.7b";
...
// ══════════════════════════════════════════════════════════
//  API CALL — через Cloudflare Worker прокси
//  ✅ fetch идёт на PROXY_URL, не напрямую на HF
//  ✅ Токен хранится в Worker — не виден в коде
//  ✅ CORS решён
// ══════════════════════════════════════════════════════════
async function generateVideo(prompt) {
  ...
  try {
    console.log(`[GenTube] Generating: "${prompt}"`);
    console.log(`[GenTube] Model: ${HF_MODEL}`);
    console.log(`[GenTube] Proxy: ${PROXY_URL}`);
    ...
    if (!response.ok) {
      ...
      if (status === 401) throw new Error("Неверный токен. Проверьте HF_TOKEN в Cloudflare Worker.");
      if (status === 403) throw new Error("Нет доступа к модели. Убедитесь что токен имеет права 'read'.");
      if (status === 404) throw new Error(`Модель "${HF_MODEL}" не найдена.`);
      if (status === 503) {
        const errData = await response.json().catch(() => ({}));
        const eta = errData.estimated_time ? `~${Math.ceil(errData.estimated_time)}s` : "немного";
        throw new Error(`Модель загружается (${eta}). Подождите и попробуйте снова.`);
      }
      if (status === 429) throw new Error("Слишком много запросов. Подождите минуту.");
      const errText = await response.text().catch(() => "Unknown error");
      throw new Error(`Ошибка API ${status}: ${errText.slice(0, 200)}`);
    }

    const contentType = response.headers.get("content-type") || "";

    if (contentType.includes("video") || contentType.includes("octet-stream")) {
      handleVideoBlob(await response.blob(), prompt);
      return;
    }
    if (contentType.includes("image/gif") || contentType.includes("image/")) {
      handleImageBlob(await response.blob(), prompt);
      return;
    }

    const json = await response.json();
    console.log("[GenTube] JSON response:", json);
    if (json.video) { handleVideoBlob(base64ToBlob(json.video, "video/mp4"), prompt); return; }
    if (Array.isArray(json) && json[0]?.blob) { handleVideoBlob(base64ToBlob(json[0].blob, "video/mp4"), prompt); return; }

    throw new Error("Неожиданный формат ответа. Проверьте консоль браузера.");
...
function handleDownload() {
  ...
  const ext = lastVideoBlob.type.includes("gif") ? "gif" : lastVideoBlob.type.includes("webm") ? "webm" : "mp4";
