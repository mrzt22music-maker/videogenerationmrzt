...
async function generateVideo(prompt) {
  ...
  try {
    console.log(`[GenTube] Generating: "${prompt}" | Style: ${selectedStyle} | Duration: ${selectedDuration}s`);
    ...
    console.log("[GenTube] Response:", data);
    ...
    if (data.video_url) {
      // Загружаем видео по URL от JSON2Video
      showToast("Видео готово! Загружаем…");
    ...
    throw new Error("Нет видео в ответе. Проверьте консоль браузера.");
